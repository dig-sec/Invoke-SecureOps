...
function Test-NetworkSecurityProtocols {
    ...
    $advancedNetworkInfo = Initialize-JsonOutput -Category "NetworkSecurityProtocols" -RiskLevel "High" -ActionLevel "Review"
    ...
    try {
        ...
        $advancedNetworkInfo.NetworkIsolation = @{
        ...
        # Add findings based on advanced network security
        if ($ipsec.Count -eq 0) {
        ...
        $advancedNetworkInfo.PublicNetworks = $publicNetworks
        $advancedNetworkInfo.DNSIssues = $dnsIssues
    }
    catch {
    ...
    if ($OutputPath) {
        Export-ToJson -Data $advancedNetworkInfo -FilePath $OutputPath
